...
const int embedding_table_size = 500000;
...
long long run_with_prefetching(const vector<float>& embedding_table, const vector<int>& input, const vector<int>& offsets) {
    ...
    const int prefetch_distance = 1;  // Prefetch 8 tokens ahead
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (int j = start_idx; j < end_idx; ++j) {
            // Prefetch multiple cache lines (64B each) of embedding vectors prefetch_distance tokens ahead
            int prefetch_pos = j + prefetch_distance;
            if (prefetch_pos < end_idx) {
                const float* prefetch_ptr = &embedding_table[input[prefetch_pos] * embedding_dim];
                // Each embedding vector is embedding_dim floats (128 floats)
                // 128 floats * 4 bytes/float = 512 bytes; 512 / 64 = 8 cache lines
                for (int offset = 0; offset < embedding_dim; offset += 16) {  // 16 floats = 64 bytes
                    _mm_prefetch(reinterpret_cast<const char*>(prefetch_ptr + offset), _MM_HINT_T0);
                }
            }

            const float* data_ptr = &embedding_table[input[j] * embedding_dim];

            // Summation without SIMD shown here is fine or SIMD can be added
            for (int d = 0; d < embedding_dim; ++d) {
                bag_embedding[d] += data_ptr[d];
            }
        }
        output.push_back(move(bag_embedding));
    }
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (int j = start_idx; j < end_idx; ++j) {
            ...
            for (int d = 0; d < embedding_dim; d += 8) {
                __m256 vec_load = _mm256_loadu_ps(data_ptr + d);
                __m256 vec_sum = _mm256_loadu_ps(&bag_embedding[d]);
                vec_sum = _mm256_add_ps(vec_sum, vec_load);
                _mm256_storeu_ps(&bag_embedding[d], vec_sum);
            }
        }
...
long long run_with_prefetching_simd(const vector<float>& embedding_table, const vector<int>& input, const vector<int>& offsets) {
    ...
    const int prefetch_distance = 4;  // Prefetch 3 tokens ahead
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (int j = start_idx; j < end_idx; ++j) {
            // Prefetch embedding vector prefetch_distance tokens ahead
            ...
            // SIMD vectorized summation of embedding_dim floats (step 8 floats)
            for (int d = 0; d < embedding_dim; d += 8) {
                __m256 vec_load = _mm256_loadu_ps(data_ptr + d);
                __m256 vec_sum = _mm256_loadu_ps(&bag_embedding[d]);
                vec_sum = _mm256_add_ps(vec_sum, vec_load);
